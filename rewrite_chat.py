...
# Replace between `class PagingModelProxy:` and `# Replace the inner model`
new_content = re.sub(r'class PagingModelProxy:.*?# Replace the inner model', new_proxy.strip() + '\n\n        # Replace the inner model', content, flags=re.DOTALL)

with open("src/sparsify/runtime/chat_generation.py", "w") as f:
    f.write(new_content)

print("Rewrote chat_generation.py successfully!")
